...
class RobotToGeomagicFeedback(Node):
    def __init__(self):
        ...
        self.declare_parameter('gravity_force_scale', 0.4)  # How much gravity you feel (0.3-0.5 = intense)
        ...
        self.get_logger().info("🔥 VEGETA MODE ACTIVATED! Prepare for intense training! 🔥")
    ...
    def compute_gravity_torques(self, joint_positions):
        ...
        if self.callback_count % 100 == 0:
            self.get_logger().info(
                f"💪 Power Level: Shoulder={torques[1]:.2f} Nm, "
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("🔥 Training complete! Power level increased! 🔥")
    finally:
        node.destroy_node()
        rclpy.shutdown()
